...
@section('content')
    ...
    {{-- Gallery --}}
    <div id="Gallery" class="swiper-gallery w-full overflow-x-auto no-scrollbar -mb-8 sm:-mb-10 px-4 sm:px-5"
        style="height: 45vh;">
        <div class="swiper-wrapper flex space-x-4 h-full">
            @foreach ($boardingHouse->rooms as $room)
                @foreach ($room->images as $image)
                    <div class="swiper-slide flex-shrink-0 max-w-xs w-full overflow-hidden rounded-lg shadow-md h-full">
    ...
    <main id="Details" class="relative flex flex-col rounded-t-[40px] py-5 pb-[10px] gap-4 bg-white z-10">
        ...
        <div id="About" class="flex flex-col gap-[6px] px-5">
            ...
            {{-- pakai seperti ini supaya bisa di confert jadi html {!!  !!} --}}
