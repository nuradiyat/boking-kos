...
@section('content')
    ...
    <div class="relative flex flex-col gap-[30px] my-[60px] px-5">
        ...
        {{-- jadi name('find-kos') akan menampilkan halaman form dan mencari ngisi data input dari name('find-kos');
lalu data terseput di kirim ke action dan terima action="{{ route('find-kos.results') }}" lalu find-kos.results akan ngecek data nya jika ada 
tampilkan atau jika maka kosong tapi tetap redirect ke  action="{{ route('find-kos.results') }} --}}
