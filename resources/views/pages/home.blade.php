{{-- kita memberi tahu kita akan menggunakan ini --}}
@extends('layouts.app')
...
@section('content')
    ...
    <div id="Categories" class="swiper w-full overflow-x-hidden mt-[30px]">
        <div class="swiper-wrapper">
            {{-- agar data terus bertambah sesuai dengan yg di database --}}
            @foreach ($categories as $category)
                <div class="swiper-slide !w-fit pb-[30px]">
                    {{-- Jadi, name('category.show') akan menampilkan halaman detail kategori berdasarkan slug yang kamu ambil dari data ($categories as $category).
                    Ketika user klik link <a href="{{ route('category.show', $category->slug) }}">, maka slug kategori tersebut dikirim ke route category.show.
                    <a href="{{ route('category.show', $category->slug Route ini menangani URL /category/{slug} dan memanggil method show di CategoryController:
                        Di method show($slug), kamu menerima parameter slug dari URL.
                    Kemudian kamu panggil repository untuk mendapatkan data kategori berdasarkan slug:
                                $category = $this->categoryRepository->getCategoryBySlug($slug);
                        Lalu ambil data kos (boarding houses) yang termasuk kategori tersebut:
                                $boardingHouses = $this->boardingHouseRepository->getBoardingHouseByCategorySlug($slug);
                                erakhir, kirim data tersebut ke view pages.category.show untuk ditampilkan. --}}
                    <a href="{{ route('category.show', $category->slug) }}" class="card">
                        <div
                            ...
                            <div class="w-[70px] h-[70px] rounded-full flex shrink-0 overflow-hidden">
                                {{-- memanggil gambar nya sebelum assets/images/thumbnails/flats.png
                            ganti jadi kalau memanggil gambar/asset sedikit berbeda 
                            {{ asset('storage/' . $category->image) }} --}}
                                <img src="{{ asset('storage/' . $category->image) }}" class="w-full h-full object-cover"
                                    alt="thumbnail">
                            </div>
                            <div class="flex flex-col gap-[2px]">
                                {{-- memanggil nama kategori --}}
                                ...
                                <div class="flex flex-col gap-3">
                                    ...
                                    <div class="flex items-center gap-[6px]">
                                        <img src="assets/images/icons/location.svg" class="w-5 h-5 flex shrink-0"
                                            alt="icon">
                                        {{-- yang di panggil itu fungsi relasi model nya bukan atribut bukan forenkey!!!! --}}
    ...
    {{-- navigation/navbar --}}
    {{-- kita panggil fungsi tplate blade componennya 
